...
for s = 1:1:size(ns,2)
    ...
    for k = 2:1:K
        t(k,1) = (k-1)*deltat;
        f1 = -u(:,k-1).*(Bx*w(:,k-1))-v(:,k-1).*(By*w(:,k-1))+Awexact(N,A,w(:,k-1),x,y,t(k-1,1),Re)/Re;
        
        w2 = w(:,k-1)+deltat*f1/2;
        p2 = A\w2;
        f2 = -(-By*p2).*(Bx*w2)-(Bx*p2).*(By*w2)+Awexact(N,A,w2,x,y,t(k-1,1)+0.5*deltat,Re)/Re;

        w3 = w(:,k-1)+deltat*f2/2;
        p3 = A\w3;
        f3 = -(-By*p3).*(Bx*w3)-(Bx*p3).*(By*w3)+Awexact(N,A,w3,x,y,t(k-1,1)+0.5*deltat,Re)/Re;

        w4 = w(:,k-1)+deltat*f3;
        p4 = A\w4;
        f4 = -(-By*p4).*(Bx*w4)-(Bx*p4).*(By*w4)+Awexact(N,A,w4,x,y,t(k,1),Re)/Re;
